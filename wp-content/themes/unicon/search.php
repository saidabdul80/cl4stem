...
<div id="page-wrap" class="blog-page search-page container">
	...
	<div id="content" class="sidebar-right twelve alt columns">
		...
		<h3><?php _e('New Search', 'unicon') ?></h3>

		<?php _e('If you are not happy with the results below please do another search', 'unicon') ?>
			...
			<article id="post-<?php the_ID(); ?>" <?php post_class('post entry-search clearfix'); ?>>
			    ...
			    <div class="entry-wrap">
			        ...
			        <div class="entry-title">
			            <h3><a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__('Permalink to %s', 'unicon'), the_title_attribute('echo=0') ); ?>" rel="bookmark"><?php the_title(); ?></a></h3>
			        </div>

			        <div class="entry-type">
			        <?php if( get_post_type($post->ID) == 'post' ){ ?>
			        	<?php echo __('Blog Post', 'unicon'); ?>
			        <?php } elseif( get_post_type($post->ID) == 'page' ){ ?>
			        	<?php echo __('Page', 'unicon'); ?>
			        <?php } elseif( get_post_type($post->ID) == 'portfolio' ){ ?>
			        	<?php echo __('Portfolio Item', 'unicon'); ?>
			        <?php } elseif( get_post_type($post->ID) == 'product' ){ ?>
			        	<?php echo __('Product', 'unicon'); ?>
			        <?php } ?>
			        </div>
		...
		<?php else : ?>
			...
			<div class="alert-message"><?php _e('No results found', 'unicon') ?></div>
