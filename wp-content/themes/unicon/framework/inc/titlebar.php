...
<?php if(rwmb_meta('minti_titlebar') != 'default' && rwmb_meta('minti_titlebar') != ''){ ?>
		...
		<?php } elseif (rwmb_meta('minti_titlebar') == 'featuredimagecenter') { ?>
			...
			<div id="fullimagecenter" class="titlebar" style="background-image: url( <?php $images = rwmb_meta( 'minti_headerimage', 'type=image_advanced&size=standard' ); if(is_array($images)){ foreach ( $images as $image ) { echo esc_url($image['url']); } } ?> );">
				<div class="container">
		...
		<?php } elseif (rwmb_meta('minti_titlebar') == 'transparentimage') { ?>
			...
			<div id="transparentimage" class="titlebar" style="background-image: url( <?php $images = rwmb_meta( 'minti_headerimage', 'type=image_advanced&size=standard' ); if(is_array($images)){ foreach ( $images as $image ) { echo esc_url($image['url']); } } ?> );">
				<div class="container">
					<div class="sixteen columns">
						<?php if(!is_single()){?><h1><?php the_title(); ?></h1><?php } else { ?> <div class="title-h1"><?php the_title(); ?></div> <?php } ?>
					</div>
...
<?php } else { ?>
		...
		<?php
			...
			elseif( is_archive() ) { 
				...
				elseif( is_tag() ) { 	$title = __('Posts Tagged:', 'unicon') . ' ' . single_tag_title('',false); }
				elseif (is_day()) { 	$title = __('Archive for', 'unicon') . ' ' . get_the_time('F jS, Y'); }
				elseif (is_month()) { 	$title = __('Archive for', 'unicon') . ' ' . get_the_time('F Y'); }
				elseif (is_year()) { 	$title = __('Archive for', 'unicon') . ' ' . get_the_time('Y'); }
				elseif (is_author()) { 	$title = __('Author Archive for', 'unicon') . ' ' . get_the_author(); }
				elseif (isset($_GET['paged']) && !empty($_GET['paged'])) { $title = __('Blog Archives', 'unicon'); }
				else{
			...
			elseif( is_404() ) { $title = __('Oops, this Page could not be found.', 'unicon'); }
