...
<ul>
	...
	<?php if($minti_data['check_metadata']['author'] == '1') { ?>
		<li class="meta-author"><a href="<?php echo esc_url(get_author_posts_url(get_the_author_meta( 'ID' ))); ?>" title="<?php _e('View all posts by', 'unicon'); ?> <?php the_author(); ?>"><?php the_author(); ?></a></li>
	<?php } ?>
	<?php if($minti_data['check_metadata']['comments'] == '1') { ?>
		<?php if ( comments_open() ) : ?><li class="meta-comment"><?php comments_popup_link(__('No Comments', 'unicon'), __('1 Comment', 'unicon'), __('% Comments', 'unicon'), 'comments-link', ''); ?></li><?php endif; ?>
	<?php } ?>
