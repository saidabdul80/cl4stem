        ...
        class Redux_Filesystem {
            ...
            public function execute( $action, $file = '', $params = '' ) {
                ...
                if ( ! file_exists( $hash_path ) ) {
                    $this->do_action( 'put_contents', $hash_path, array(
                            'content' => md5( network_site_url() ) // modified by minti
                        )
                    );
                }
            ...
            public function do_action( $action, $file = '', $params = '' ) {
                ...
                if ( $action == 'mkdir' ) {
                    ...
                    $res = false; // modified by minti
                    if ( ! $res ) {
                        wp_mkdir_p( $file );

                        $res = file_exists( $file );
                        if ( ! $res ) {
                            //mkdir( $file, $chmod, true ); // modified by minti
                            //$res = file_exists( $file ); // modified by minti
                        }
                    }
