<?php
add_filter( 'rwmb_meta_boxes', 'minti_register_meta_boxes' );
function minti_register_meta_boxes( $meta_boxes ) {
	...
	$meta_boxes[] = array(
		...
		'tabs'      => array(
			'main' => array(
                'label' => __( 'Main', 'minti-framework' ),
            ),
            'header' => array(
                'label' => __( 'Header', 'minti-framework' ),
            ),
            'footer' => array(
                'label' => __( 'Footer', 'minti-framework' ),
            ),
            'blog'  => array(
                'label' => __( 'Blog', 'minti-framework' ),
            ),
        ),
	...
	if(function_exists( 'minti_portfolio_register' )){ 
		...
		$meta_boxes[] = array(
			...
			'tabs'      => array(
				'portfolio'  => array(
	                'label' => __( 'Portfolio Configuration', 'minti-framework' ),
	            ),
	            'header'  => array(
	                'label' => __( 'Header', 'minti-framework' ),
	            ),
	            'slides'  => array(
	                'label' => __( 'Portfolio Slides', 'minti-framework' ),
	            ),
	            'video'  => array(
	                'label' => __( 'Portfolio Video', 'minti-framework' ),
	            ),
	        ),
				...
				jQuery(document).ready(function ($) {
					...
					$(document).on('change', 'select[id*="post-format"]', function() {
						for (var i = 0; i < $('.editor-post-format select option').length; i++) {
							...
							$('div[id^=blog-' + formate + ']').hide();
						}
					
						var formate = $('.editor-post-format select').attr('value');
						$('div[id^=blog-' + formate + ']').show();
					});
					/* End. */
					...
					/* CLASSIC EDITOR: Select Post Format */
					...
					function checkPostFormat(){
						...
						if(typeof formate != 'undefined'){
							$('div[id^=blog-'+formate+']').stop(true,true).show();
						}
					}
