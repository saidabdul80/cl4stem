			...
			if ( ! empty( $this->parent ) && preg_match( '/' . preg_quote( '[' . $this->field['id'] . ']' ) . '/', $this->parent ) ) {
				...
				if ( ! empty( $this->value ) ) {
					...
					foreach ( $this->value as $value ) {
						...
						echo ( $title_prefix ) ? '<span class="spf-cloneable-title-prefix">' . $title_prefix . '</span>' : '';
						echo ( $title_auto ) ? '<span class="spf-cloneable-value">' . $first_value . '</span>' : '';
						echo '</span>';
						echo '</h4>';
				...
				echo '<div class="spf-cloneable-alert spf-cloneable-max">' . esc_html__( 'You can not add more than', 'smart-post-show' ) . ' ' . $args['max'] . '</div>';
				echo '<div class="spf-cloneable-alert spf-cloneable-min">' . esc_html__( 'You can not remove less than', 'smart-post-show' ) . ' ' . $args['min'] . '</div>';
